...
{
#ifdef __SWITCH__
    // Switch HID enumerates 8 player slots regardless of physical connection,
    // so SDL_NumJoysticks() always reports 8. Use SDL_IsGameController() to
    // count actually-attached pads, and coerce any inherited keyboard / mouse
    // / out-of-range joystick selection back to AI.
    {
        int real_pads = 0;
        for (int i = 0; i < SDL_NumJoysticks(); i++) {
            if (SDL_IsGameController(i)) real_pads++;
        }
        bool changed = false;
        for (int i = 0; i < 8; i++) {
            int t = Config::bomber[i].get_controller();
            bool invalid = (t == Controller::KEYMAP_1 || t == Controller::KEYMAP_2 ||
                            t == Controller::KEYMAP_3 || t == Controller::RCMOUSE);
            if (!invalid && t >= Controller::JOYSTICK_1 && t <= Controller::JOYSTICK_8) {
                int joy_idx = t - Controller::JOYSTICK_1;
                if (joy_idx >= real_pads) invalid = true;
            }
            if (invalid) {
                Config::bomber[i].set_controller(Controller::AI);
                changed = true;
            }
        }
...
void PlayerSetup::handle_enter()
{
    ...
    case 3:
    {
        int next = Config::bomber[cur_row].get_controller() + 1;
#ifdef __SWITCH__
        // Skip KEYMAP_1..3, RCMOUSE, and JOYSTICK_N for slots that have no
        // physically attached controller. SDL_NumJoysticks() reports 8 on
        // Switch (HID slot count), so the actual count comes from probing
        // SDL_IsGameController() per index.
        int real_pads = 0;
        for (int i = 0; i < SDL_NumJoysticks(); i++) {
            if (SDL_IsGameController(i)) real_pads++;
        }
        int max = 6 + SDL_NumJoysticks();  // matches set_controller's modulo
        if (max < 1) max = 6;

        auto is_invalid = [&](int v) -> bool {
            if (v == Controller::KEYMAP_1 || v == Controller::KEYMAP_2 ||
                v == Controller::KEYMAP_3 || v == Controller::RCMOUSE) {
                return true;
            }
            if (v >= Controller::JOYSTICK_1 && v <= Controller::JOYSTICK_8) {
                int joy_idx = v - Controller::JOYSTICK_1;
                if (joy_idx >= real_pads) return true;
            }
            return false;
        };
        ...
        next = next % max;
        int safety = 0;
        while (is_invalid(next) && safety++ < max) {
            next = (next + 1) % max;
        }
#endif
        Config::bomber[cur_row].set_controller(next);
        break;
    }
    }
}
